...
} from '@tylertech/tyler-components-web';
import { isArray, isString, isDefined } from '@tyler-components-web/core';

import { Utils } from 'src/utils';
import { ListDropdownHeaderBuilder } from '@tylertech/tyler-components-web/typings/list-dropdown';
...
export class AutocompleteRangeComponent implements ControlValueAccessor, AfterViewInit, OnDestroy {
    ...
    return new Promise((resolve, reject) => {
      this.optionFilter(this.filter).subscribe(
        (response) => {
          ...
          // resolve([{ options: [], builder: this.rangeBuilder }, { options }]);
          resolve(options);
        },
        () => reject()
      );
  ...
  // public rangeBuilder: AutocompleteOptionGroupBuilder = (option: IAutocompleteOptionGroup): HTMLElement => this.rangeRef.rootNodes[0] as HTMLElement;
  public optionHeaderBuilder: ListDropdownHeaderBuilder = (): HTMLElement => this.rangeRef.rootNodes[0] as HTMLElement;
  ...
  writeValue(values: Array<IOption> | Array<string | string[] | number>): void {
    ...
    if (isArray(values)) {
      values.forEach((o) => {
        if (isArray(o) || isArray(o.value)) {
          const rangeOption = isDefined(o.value) ? o.value : o;
    ...
    window.requestAnimationFrame(() => {
      this.renderer.setProperty(this.autocompleteRef.nativeElement, 'value', options);
    });
  }

  ngAfterViewInit(): void {
    window.requestAnimationFrame(() => {
      this.rangeRef = this.viewContainerRef.createEmbeddedView(this.rangeTemplateRef);
  ...
  public autocompleteChange(): void {
  ...
  public addRangeOption(): void {
    ...
    this.filterInputRef.nativeElement.value = this.selectedTextBuilder(this.autocompleteRef.nativeElement.value);
  }

  public deleteRangeOption(option: IOption): void {
