...
export class QueryBuilderComponent implements OnInit {
  ...
  public addFilter(formGroup: FormGroup, type: 'condition' | 'group') {
    if (formGroup === this.formGroup) {
      ...
      switch (type) {
        case 'condition':
          ...
          break;
        case 'group':
          let filterFormArray = formGroup.get('filters') as FormArray;
          if (!filterFormArray) {
            filterFormArray = new FormArray([]);
            formGroup.addControl('filters', filterFormArray);
          }
          filterFormArray.push(new FormArray([this.buildFilterFormGroup()]));
          break;
      }
    }
  }

  public deleteFilter(formGroup: FormGroup) {
    ...
    if (formArray.controls.length === 0) {
      const parentIndex = (formArray.parent as FormArray).controls.indexOf(formArray);
      (formArray.parent as FormArray).removeAt(parentIndex);
    }
  }
